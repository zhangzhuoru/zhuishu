...
<script>
	...
	export default {
		...
		created() {
			...
			if(this.$route.params.paihid) {
				this.paihid = this.$route.params.paihid;
			}
			if(!this.$route.params.paihid) {
				this.paihid = this.$store.state.paihid
				console.log('this.paihid ', this.paihid)
			}

			const url = '/api/ranking/' + this.paihid

			this.axios.get(url).then(response => {
				this.zjia = response.data.ranking.books
				this.title = response.data.ranking.title
